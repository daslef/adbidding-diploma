...
const BiddingInterface = () => {
  ...
  const bidMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success("Ставка сделана", {
        description: `Ставка размером ${formatCurrency(parseInt(bidAmount))} ₽ сделана.`,
      });
    },
    onError: (error: any) => {
    ...
    if (socket) {
      ...
      socket.on("new-bid", (data) => {
        ...
        if (user && user.id !== data.bid.userId && isWatching) {
          toast.info("Сделана новая ставка", {
            description: `Новая ставка размером ${formatCurrency(data.bid.amount)} ₽ была размещена.`,
          });
        }
      });
  ...
  const handleBidSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!adSpot || !user) {
      toast.error("Невозможно сделать ставку", {
        description: user
          ? "Информация об аукционе недоступна"
    ...
    if (amount % 5 !== 0) {
      toast.error("Некорректный шаг ставки", {
        description: "Ставки должны быть кратны 5,000 ₽",
      });
      return;
    }

    if (amount <= adSpot.currentPrice) {
      toast.error("Ставка недостаточна", {
        description: `Ставка должна превышать текущую: ${formatCurrency(adSpot.currentPrice)} ₽`,
      });
      return;
    }
  ...
  const minBid = adSpot.currentPrice + 5_000;
              ...
              <div className="p-6">
                <div className="flex justify-between items-start">
                  ...
                  >
                    ...
                    {isWatching ? "Не интересует" : "Наблюдать"}
                  </button>
                </div>
                  ...
                  {[
                    {
                      icon: Calendar,
                      label: "Событий",
                      value: adSpot.eventCount,
                    },
                    { icon: Trophy, label: "Локация", value: adSpot.location },
                  ].map((metric, index) => (
                    <div
                      key={index}
                      className="p-4 bg-secondary rounded-lg transition-colors duration-300"
                    >
                      ...
                      <p
                        className={`${typeof metric.value === "number" ? "text-2xl" : "text-lg"} font-bold mt-1 text-foreground`}
                      >
                        {metric.value}
                      </p>
                    </div>
            ...
            <div className="bg-card rounded-xl shadow-lg p-6 transition-colors duration-300">
              ...
              <form onSubmit={handleBidSubmit} className="space-y-4">
                <div className="flex gap-4">
                  <input
                    ...
                    placeholder={`Минимальная ставка: ${formatCurrency(minBid)}`}
                ...
                <div className="text-xs text-muted-foreground">
                  Ставки должны быть кратны 5,000 ₽. Минимальная ставка:
                  {formatWholeNumber(minBid)} ₽
                </div>
              </form>
              ...
              <AnimatePresence>
                {showBidSuccess && (
                  ...
                  >
                    <Alert>
                      <AlertDescription>Ставка сделана!</AlertDescription>
                    </Alert>
                  </motion.div>
                )}
              </AnimatePresence>
