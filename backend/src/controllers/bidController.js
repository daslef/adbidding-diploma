const { Bid, AdSpot, User } = require('../models');
...
/**
 * Get all bids for an ad spot
 * @route GET /api/adspots/:id/bids
 * @access Public
 */
exports.getBidsForAdSpot = asyncHandler(async (req, res) => {
  const { id } = req.params;
 ...
 * @access Private
 */
exports.getUserBids = asyncHandler(async (req, res) => {
 ...
 * @access Public
 */
exports.getHighestBids = asyncHandler(async (req, res) => {
...
exports.placeBid = asyncHandler(async (req, res) => {
  ...
  if (currentCount > 5) {
    throw new BadRequestError('You are bidding too frequently. Please wait a minute.');
  }
  ...
  if (amount <= adSpot.currentPrice) {
    throw new BadRequestError(`Bid must be higher than current price: $${adSpot.currentPrice}`);
  }
  ...
  // Update previous highest bid - FIXED: proper Sequelize operator syntax
  await Bid.update(
    { isHighestBid: false },
    { 
      where: { 
